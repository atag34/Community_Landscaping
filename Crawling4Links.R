...
url <- results[which(!is.na(str_extract(results, url_pattern)))][1]
url <- domain(url)
  return(url)
}
...
getalllinks <- function(g_url){
  links <- unique(html_attr(html_nodes(read_html(paste0("https://www.",g_url)), "a"), "href"))
  
  if (length(links)==0){
    links <- unique(html_attr(html_nodes(read_html(paste0("http://www.",g_url)), "a"), "href"))
    links <- links[which((startsWith(links, "/") & !startsWith(links,"//")) | grepl(g_url,links))]
    links <- ifelse(grepl(g_url,links),links,paste0(g_url,links))
  }else{
    links <- links[which((startsWith(links, "/") & !startsWith(links,"//")) | grepl(g_url,links))]
    links <- ifelse(grepl(g_url,links),links,paste0(g_url,links))}
...
mycorp <- Corpus(VectorSource(all))
